...
Emitter::Emitter(const EmitterData& data_, Particle* model_, int id_)
  : data(data_)
  , model(model_)
  , emitAccumulator(0.0)
  , active(true)
...
{
  if (!active || data.emitRate <= 0.0f || !model)
    return;
  ...
  for (int i = 0; i < toEmit; ++i) {
    ...
    if (data.velDist == VelocityDistribution::GAUSSIAN) {
      std::normal_distribution<float> nx(data.velMean.x, data.velEstDev.x);
      std::normal_distribution<float> ny(data.velMean.y, data.velEstDev.y);
      std::normal_distribution<float> nz(data.velMean.z, data.velEstDev.z);
...
int ParticleSystem::addEmitter(const EmitterData& cfg, Particle* model)
{
  int id = nextEmitterId++;
  emitters.emplace_back(cfg, model, id);
  return id;
}
...
void ParticleSystem::updateEmitterPosition(int id, const Vector3& pos)
{
  for (auto& e : emitters) {
    if (e.getId() == id) {
      e.setPosition(pos);
...
void ParticleSystem::updateEmitterData(int id, const EmitterData& data)
{
  for (auto& e : emitters) {
    if (e.getId() == id) {
      e.updateData(data);
...
bool ParticleSystem::isEmitterActive(int id) const
{
